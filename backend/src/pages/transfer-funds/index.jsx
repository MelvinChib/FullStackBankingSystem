...
const TransferFunds = () => {
  ...
  useEffect(() => {
    ...
    const initialAlerts = [
      {
        ...
        message: 'Daily transfer limit: $10,000 | Monthly limit: $50,000',
        dismissible: true
      }
    ];
    ...
    if (now >= maintenanceStart && now <= maintenanceEnd) {
      initialAlerts?.unshift({
        ...
        message: 'External transfers may be delayed during maintenance window (2:00 AM - 4:00 AM EST)',
        dismissible: false
      });
    }
  ...
  const handleConfirmTransfer = (confirmedData) => {
    ...
    setAlerts(prev => [{
      ...
      message: `Your ${result?.transferType} transfer of $${parseFloat(result?.amount)?.toLocaleString()} has been processed.`,
      dismissible: true
    }, ...prev]);
  };
