...
import { defaultRolesForEmail, permissionsForRoles } from '../../services/permissions';
...
const Login = () => {
  ...
  const handleLogin = async (formData) => {
    ...
    try {
      ...
      const mockCredentials = getDemoCredentials();
      const isDemoLogin = mockCredentials && formData?.email === mockCredentials.email && formData?.password === mockCredentials.password;
      ...
      // Call backend login for real users
  ...
  const handleTwoFactorVerify = async (code) => {
    ...
    try {
      ...
      // For the demo flow, allow selecting admin by email pattern
      const roles = defaultRolesForEmail(mockCredentials?.email);
      const permissions = permissionsForRoles(roles);
      handleLoginSuccess({ email: mockCredentials?.email, roles, permissions });
    } catch (err) {
  ...
  const handleLoginSuccess = (userData) => {
    const roles = userData?.roles || defaultRolesForEmail(userData?.email);
    const permissions = userData?.permissions || permissionsForRoles(roles);

    // Store session data
    const sessionData = {
      user: {
        id: userData?.backendUser?.id || '1',
        name: userData?.backendUser?.name || 'John Smith',
        email: userData?.email,
        accountNumber: userData?.backendUser?.accountNumber || '****1234',
      ...
      <header className="bg-card border-b border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
              ...
              <div className="w-10 h-10 bg-[#0E7C6B] rounded-lg flex items-center justify-center">
                <Icon name="Building2" size={24} color="white" />
              </div>
              <div className="flex flex-col">
                <span className="text-lg font-bold text-gray-900 leading-tight">MelCredit Union</span>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-card border border-border rounded-lg p-6">
                <h3 className="text-lg font-semibold text-foreground mb-4">
                  Why Bank with MelCredit Union Bank?
                </h3>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div>
              <div className="flex items-center space-x-3 mb-4">
                ...
                <span className="font-semibold text-foreground">MelCredit Union Bank</span>
              </div>
            ...
            <div>
              ...
              <div className="space-y-2 text-sm text-muted-foreground">
                ...
                <div className="flex items-center space-x-2">
                  ...
                  <span>support@standardchartered.com</span>
                </div>
                <div className="flex items-center space-x-2">
          ...
          <div className="border-t border-border mt-8 pt-6 flex flex-col sm:flex-row justify-between items-center">
            <p className="text-xs text-muted-foreground">
              © {new Date()?.getFullYear()} MelCredit Union Bank. All rights reserved.
            </p>
